                        ...
                        uint32_t color, float x, float y, float w, float h)
{
    if (w <= 0.0f || h <= 0.0f)
        return;
    gs_effect_set_color(color_param, argb(color));
    gs_matrix_push();
    gs_matrix_translate3f(x, y, 0.0f);
    while (gs_effect_loop(effect, "Solid"))
        gs_draw_sprite(nullptr, 0, (uint32_t)std::ceil(w), (uint32_t)std::ceil(h));
    gs_matrix_pop();
}
                       ...
                       float x, float y, float w, float h, float db)
{
    ...
    const float step = 1.0f / std::max(1, m->segments);
    const float lit_segments = frac / step;
    ...
    for (int i = 0; i < m->segments; ++i) {
        ...
        const float size = std::max(0.0f, pos1 - pos0 - (float)m->gap);
        const bool on = (float)i < lit_segments;
        const float segment_db = m->min_db + ((float)i + 0.5f) / m->segments * (-m->min_db);
...
static void vu_render(void *data, gs_effect_t *)
{
    ...
    if (!effect || !color_param)
        return;

    render_rect(effect, color_param, m->color_background, 0, 0, (float)cx, (float)cy);

    const float outer = 6.0f;
    if (m->direction == 0) {
        const float bar_h = std::max(1.0f, ((float)cy - outer * 3.0f) / 2.0f);
        const float bar_w = (float)cx - outer * 2.0f;
        render_bar(m, effect, color_param, outer, outer, bar_w, bar_h, peak[0]);
        render_bar(m, effect, color_param, outer, outer * 2.0f + bar_h, bar_w, bar_h, peak[1]);

        if (m->show_hold) {
            std::lock_guard<std::mutex> lock(m->level_mutex);
            const float hold_w = std::max(1.0f, ((float)cx - outer * 2.0f) * level_to_fraction(m->hold[0], m->min_db));
            const float hold_y = outer + bar_h - 2.0f;
            render_rect(effect, color_param, m->color_peak, outer + hold_w - 1.0f, hold_y, 2.0f, 2.0f);
            const float hold_w2 = std::max(1.0f, ((float)cx - outer * 2.0f) * level_to_fraction(m->hold[1], m->min_db));
            const float hold_y2 = outer * 2.0f + bar_h + bar_h - 2.0f;
            render_rect(effect, color_param, m->color_peak, outer + hold_w2 - 1.0f, hold_y2, 2.0f, 2.0f);
        }
    } else {
        const float bar_w = std::max(1.0f, ((float)cx - outer * 3.0f) / 2.0f);
        const float bar_h = (float)cy - outer * 2.0f;
        render_bar(m, effect, color_param, outer, outer, bar_w, bar_h, peak[0]);
        render_bar(m, effect, color_param, outer * 2.0f + bar_w, outer, bar_w, bar_h, peak[1]);
    }
}
